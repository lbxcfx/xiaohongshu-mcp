	...
	if len(filters) > 0 {
		...
		for _, filter := range allInternalFilters {
			selector := fmt.Sprintf(
				`div.filter-panel div.filters:nth-child(%d) div.tags:nth-child(%d)`,
				filter.FiltersIndex,
				filter.TagsIndex,
			)
...
func pageShowsSearchLoginPrompt(page *rod.Page) bool {
	result, err := page.Eval(`() => document.body ? document.body.innerText.slice(0, 1200) : ""`)
	if err != nil {
		return false
	}
	return containsLoginPrompt(result.Value.String())
}

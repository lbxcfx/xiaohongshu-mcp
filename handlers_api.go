...
type phoneLoginSendCodeRequest struct {
	Phone string `json:"phone" binding:"required"`
}

type phoneLoginVerifyCodeRequest struct {
...
func (s *AppServer) startPhoneLoginHandler(c *gin.Context) {
	result, err := s.xiaohongshuService.StartPhoneLogin(c.Request.Context())
	if err != nil {
		respondError(c, http.StatusInternalServerError, "START_PHONE_LOGIN_FAILED",
			"启动手机号登录失败", err.Error())
		return
	}

	respondSuccess(c, result, "启动手机号登录成功")
}

func (s *AppServer) startLoginSessionHandler(c *gin.Context) {
	result, err := s.xiaohongshuService.StartLoginSession(c.Request.Context())
	if err != nil {
		respondError(c, http.StatusInternalServerError, "START_LOGIN_SESSION_FAILED",
			"启动登录会话失败", err.Error())
		return
	}

	respondSuccess(c, result, "启动登录会话成功")
}

func (s *AppServer) sendPhoneLoginCodeHandler(c *gin.Context) {
	var req phoneLoginSendCodeRequest
	...
	result, err := s.xiaohongshuService.SendPhoneLoginCode(c.Request.Context(), req.Phone)
	if err != nil {
		respondError(c, http.StatusInternalServerError, "SEND_PHONE_CODE_FAILED",
			"发送手机号验证码失败", err.Error())
		return
	}

	respondSuccess(c, result, "发送手机号验证码成功")
}

func (s *AppServer) verifyPhoneLoginCodeHandler(c *gin.Context) {
	var req phoneLoginVerifyCodeRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, "INVALID_REQUEST",
			"请求参数错误", err.Error())
		return
	...
	result, err := s.xiaohongshuService.VerifyPhoneLoginCode(c.Request.Context(), req.Code)
	if err != nil {
		respondError(c, http.StatusInternalServerError, "VERIFY_PHONE_CODE_FAILED",
			"提交手机号验证码失败", err.Error())
		return
	}

	respondSuccess(c, result, "提交手机号验证码成功")
}

func (s *AppServer) checkSearchAccessHandler(c *gin.Context) {
	keyword := c.DefaultQuery("keyword", "医美")

	result, err := s.xiaohongshuService.CheckSearchAccess(c.Request.Context(), keyword)
	if err != nil {
		if err == xhsErrors.ErrSearchLoginRequired {
			respondError(c, http.StatusUnauthorized, "SEARCH_LOGIN_REQUIRED",
				"搜索页需要重新登录", err.Error())
			return
		}
		respondError(c, http.StatusInternalServerError, "CHECK_SEARCH_ACCESS_FAILED",
			"检查搜索授权失败", err.Error())
		return
	}

	respondSuccess(c, result, "检查搜索授权成功")
}
